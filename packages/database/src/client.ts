    ...
    try {
      const [result] = (await client`
        SELECT
          ...
          AND to_regclass('public.agent_run_steps_run_id_sequence_unique') IS NOT NULL
          AND to_regclass('public.agent_run_steps_run_id_step_name_unique') IS NOT NULL
          ...
          AND EXISTS (
            ...
            WHERE table_schema = 'public'
              AND table_name = 'job_email_analyses'
              AND column_name = 'meeting_url_type'
              AND is_nullable = 'NO'
          )
          AND EXISTS (
            SELECT 1
          ...
          AND EXISTS (
            ...
            WHERE table_schema = 'public'
              AND table_name = 'oauth_authorization_states'
              AND column_name = 'authorization_purpose'
              AND is_nullable = 'NO'
          )
          AND EXISTS (
            SELECT 1
