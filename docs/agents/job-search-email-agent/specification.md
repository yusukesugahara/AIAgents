...
Gmailに届いた就職活動関連メールを取得し、OpenAI Responses APIのFunction CallingとStructured Outputsで分類・情報抽出します。返信が必要で、安全条件を満たす場合は、Function Calling経由で元スレッドのGmail下書きを作成します。
...
上限付きのTool Useループと決定的な安全Policyを組み合わせて実装します。LLMは意味理解、構造化抽出、ツール選択を担当し、外部サービスへ書き込む可否はZodで検証済みの値とTypeScriptの条件分岐で決定します。

```text
Gmailから対象メールとスレッドを取得
  ↓
OpenAI APIが読取Toolを呼び、アプリがfunction_call_outputを返す
  ↓
Zod Schemaで検証・正規化
  ↓
返信とCalendarの安全条件を判定
  ├── 返信可能       → 下書きToolを許可 → Policy再検証 → Gmail下書き
...
| `packages/llm` | OpenAI Responses API、Structured Outputs、自前Tool Useループ |
...
OpenAI Responses APIへ`get_email_thread`と`get_agent_context`を公開し、両方のFunction Callingを必須にします。アプリがツールを実行して`function_call_output`を返した後、Structured Outputsで解析結果を受け取ります。解析結果には次を含みます。
...
通常返信では、LLMが`create_reply_draft`へ本文・信頼度・警告を渡します。日程調整では、LLMが引数なしの`create_scheduling_placeholder_draft`を呼び、アプリケーション管理の固定文面を使用します。`scheduling_request`は不足情報があっても、利用者が候補日時を入力できるプレースホルダー付き下書きを作成できます。

下書きToolの実行時に、設定、解析結果、信頼度、警告、返信ヘッダー、最新スレッド、冪等性を再検証します。宛先、Thread ID、Message IDはツール引数にせず、検証済みのアプリケーション状態を使用します。送信Toolは定義しません。
...
- Function Callingのツール名と引数を検証し、未知のツール、引数不正、回数・ターン上限超過を拒否します。
...
- Unit Test: Function Callingループ、Schema、Prompt、返信・Calendar条件、Run Step、Connector、Repository
