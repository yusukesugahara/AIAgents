# ADR-0003: PostgreSQL 18.4とDrizzle ORMを採用する
...
新規開発であるため、長いサポート期間とPostgreSQL 18の標準機能を利用できる構成を優先します。

## Decision

- DatabaseにPostgreSQL 18.4を採用する
- ORMにDrizzle ORMを採用する
- PostgreSQL Driverに`postgres.js`を採用する
- Migration生成にDrizzle Kitを使用する
- 内部主キーにはPostgreSQL 18の`uuidv7()`を原則使用する
- ORM利用は`packages/database`へ限定する
- エージェントはRepository interfaceへ依存する
- 複雑なQueryではDrizzleのSQL TemplateまたはSQL Migrationを使用する
- ローカル環境とCIはPostgreSQL 18.4へ固定する

## PostgreSQL 18 Docker rules

PostgreSQL 18の公式Docker Imageでは、永続Volumeを次へマウントします。
...
PostgreSQL 17以前で一般的だった`/var/lib/postgresql/data`へのVolumeマウントを、新規構成では使用しません。

本番環境では、可能な限りPostgreSQL 18対応のマネージドサービスを利用します。利用予定のサービスが18に未対応の場合だけ、本番環境を一時的に17とし、18対応後のアップグレード計画を同時に作成します。

## UUID rules

時系列性が必要な内部IDには`uuidv7()`を使用します。
...
GmailやGoogle Calendarなどの外部サービスIDはUUIDへ変換せず、文字列として専用カラムへ保存します。
...
- ORMから生SQLへ段階的に下りられる
- PostgreSQL 18の長いサポート期間を利用できる
- `uuidv7()`を拡張なしで使用できる
- 時系列に近いUUIDにより、ランダムUUIDよりIndex局所性を期待できる
...
- 本番のマネージドDBがPostgreSQL 18へ未対応の可能性がある
- PostgreSQL 17以前とDocker Volumeの配置が異なる
...
- PostgreSQL 18.4コンテナを使用してMigration Testを行う
- `uuidv7()`の利用箇所をMigration Testで検証する
...
安定した選択肢ですが、新規開発であるため、より長いサポート期間と`uuidv7()`の標準搭載を優先して18.4を採用します。
